...
static int nfcc_hw_check(struct i2c_client *client, unsigned int enable_gpio, unsigned int
firm_gpio)
{
	int ret = 0;
    //modify by baojian for nfcc firmware damaged
    //unsigned char raw_nci_reset_cmd[] =  {0x20, 0x00, 0x01, 0x00};
    unsigned char raw_nci_recover_cmd[] = {0x00,0x04,0xF2,0x00,0x00,0x00,0xF5,0x33};
    ...
    ret = i2c_master_send(client, raw_nci_recover_cmd,
                            sizeof(raw_nci_recover_cmd));
        //end by baojian
	if (ret < 0) {
		dev_err(&client->dev,
