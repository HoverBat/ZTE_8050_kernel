...
static volatile int reset_gpio;
static int major;
static int is_por_off;
static struct mmc_host *g_mmc;
static struct delayed_work por_dw;
...
static int ssx1207_por_gpio_control(int gpio, int val)
{	
	printk("%s val=%d enter \n", __func__, val);
...
static long por_ctl_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	...
	case POR_ON:
		is_por_off = 0;
		ssx1207_por_gpio_control(reset_gpio, 1);
		schedule_delayed_work(&por_dw, HZ*5);
		break;
		
	case POR_OFF:
		is_por_off = 1;
		ssx1207_por_gpio_control(reset_gpio, 0);
		g_mmc->caps |= MMC_CAP_NEEDS_POLL;
		mmc_detect_change(g_mmc, 0);
		break;
...
static ssize_t por_ctl_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	int val, ret;
...
static struct file_operations por_ctl_fops = {
	.owner  =   THIS_MODULE,
	.open   =   por_ctl_open,
	.write = por_ctl_write,
...
void delay_work_function(struct work_struct *work)
{
	g_mmc->caps &= ~MMC_CAP_NEEDS_POLL;
}
...
static int por_ctl_init(const char *pdev_name)
{
	...
	por_ctl_dev = (struct class_device *)device_create(por_ctl_class, NULL, MKDEV(major, 0), NULL, pdev_name);
	INIT_DELAYED_WORK(&por_dw, delay_work_function);
	
	return 0;
}


static int ssx1207_init_config(struct spi_device *spi)
{
	struct device *dev = &spi->dev;
	struct device_node *np = dev->of_node;
	int ret = 0;

	printk("ssx1207 %s enter \n", __func__);
	if (!np){
		printk("ssx1207 %s spidev has null point \n", __func__);
		return -EINVAL;
	}

	/* reset gpio config */
	reset_gpio = of_get_named_gpio(np, "reset-gpios", 0);
	printk("ssx1207 reset_gpio:%d %s enter \n", reset_gpio, __func__);
	if (reset_gpio < 0) {
		printk("don't find reset-gpio ret=%d\n", reset_gpio);
		return 0;
	}
	
	ret = gpio_request(reset_gpio, "ssx1207 reset");
	printk("ssx1207 ret:%d %s enter \n", ret, __func__);
	if (ret) {
...
checkstatus:
	...
	case MMC_RSP_SPI_R2:
		...
		if (MMC_SEND_STATUS == cmd->opcode && is_por_off) {
			value = -1;
		}
